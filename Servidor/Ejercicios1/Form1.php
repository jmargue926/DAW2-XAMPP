...
<body>
	...
	<?php
		...
		if($comunidad == '-')
			...
			echo "<p> Ha seleccionado la cominidad: " . $comunidad . "</p>\n";

		$comunidad = $_REQUEST["comunidad"];
		if($nIdiomas == 0){
			echo "<p>No ha seleccionao ningún idioma!</p>\n";
		}else{
			echo "<p>Ha seleccionado los siguientes idiomas: </p>\n";
			for($i = 0; $i < $nIdiomas; $i++)
				echo "<li> " . $idiomas[$i] . " </li>\n";
		}

		if(count($publi) == 0){
			echo "<p>No ha aceptado publicidad :( </p>\n";
		}else{
			echo "<p>Ha aceptado publicidad :D </p>\n";
		}

		echo "<p>Otra info introducida: <br> " . $otros;
	?>
</body>
</html>